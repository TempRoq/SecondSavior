...
public class ActionHandler : MonoBehaviour
{
    ...
    public void PerformAction(Action a, bool newfacingRight = false)
    {
    ...
    public void BuildHitboxes() //Place the hitboxes into the scene and see if they interact with anything 
    {
        ...
        foreach (Hitbox h in currCluster.hitboxes)
        {
            ...
            if (h.dimensions.z == 0) //Use Physics 2D
            {
                Collider2D[] cols = Physics2D.OverlapBoxAll(transform.position + new Vector3(h.offsetFromAnchor.x * (currMoveFacingRight ? 1 : -1), h.offsetFromAnchor.y), h.dimensions / 2, 0f, targetsToHit);
